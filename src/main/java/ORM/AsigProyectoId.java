package ORM;
// Generated 18 nov. 2020 20:00:38 by Hibernate Tools 5.4.21.Final

import java.util.Date;

/**
 * AsigProyectoId generated by hbm2java
 */
public class AsigProyectoId implements java.io.Serializable {

	private String dniEmp;
	private int idProy;
	private Date FInicio;

	public AsigProyectoId() {
	}

	public AsigProyectoId(String dniEmp, int idProy, Date FInicio) {
		this.dniEmp = dniEmp;
		this.idProy = idProy;
		this.FInicio = FInicio;
	}

	public String getDniEmp() {
		return this.dniEmp;
	}

	public void setDniEmp(String dniEmp) {
		this.dniEmp = dniEmp;
	}

	public int getIdProy() {
		return this.idProy;
	}

	public void setIdProy(int idProy) {
		this.idProy = idProy;
	}

	public Date getFInicio() {
		return this.FInicio;
	}

	public void setFInicio(Date FInicio) {
		this.FInicio = FInicio;
	}

	public boolean equals(Object other) {
		if ((this == other))
			return true;
		if ((other == null))
			return false;
		if (!(other instanceof AsigProyectoId))
			return false;
		AsigProyectoId castOther = (AsigProyectoId) other;

		return ((this.getDniEmp() == castOther.getDniEmp()) || (this.getDniEmp() != null
				&& castOther.getDniEmp() != null && this.getDniEmp().equals(castOther.getDniEmp())))
				&& (this.getIdProy() == castOther.getIdProy())
				&& ((this.getFInicio() == castOther.getFInicio()) || (this.getFInicio() != null
						&& castOther.getFInicio() != null && this.getFInicio().equals(castOther.getFInicio())));
	}

	public int hashCode() {
		int result = 17;

		result = 37 * result + (getDniEmp() == null ? 0 : this.getDniEmp().hashCode());
		result = 37 * result + this.getIdProy();
		result = 37 * result + (getFInicio() == null ? 0 : this.getFInicio().hashCode());
		return result;
	}

}
